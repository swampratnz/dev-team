...
`?repo=`/`?mode=`/`?state=`/`?story_id=` narrow the list by exact string
match against the job's `repo` (as originally submitted, or resolved from
`meta.json` for a `verify` job — see `_verify_spec`), `mode`
(`assess`/`deliver`/`verify`), `state`
(`queued`/`running`/`succeeded`/`failed`/`cancelled`), and `story_id` (the
foreman-enqueued job's originating backlog story id — `None` for a plain,
non-foreman job, which never matches a non-empty `?story_id=`)
respectively. All four are optional, AND-composed with each other and with
...
an unrecognised value (a typo'd mode, a state outside the closed set, an
unknown story id) is never rejected; it simply matches zero jobs. There is
no normalization between repo forms (e.g. `owner/name` vs. a full URL are
distinct strings) — a caller filters with the exact string a job was
submitted with.
...
for that job), never an error. Errors:
