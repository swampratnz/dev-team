...
def test_security_scan_missing_binary_for_php_profile_degrades_gracefully():
    # The #50 missing-binary degrade path composes with the new PHP
    # security_scan_command with zero new engine.py code: composer absent
    # must not be misread as scanner findings.
    ws = InMemoryWorkspace({"composer.json": "{}"})
    cmd = GateCycleRunner()
    cmd.add_rule(
        "composer audit",
        CommandResult(
            ...
            EXIT_NOT_FOUND,
            "",
            "[Errno 2] No such file or directory: 'composer'",
        ),
    )
    runner = ScriptedRunner(by_system_prompt=engine_responses())
    engine = _engine(runner, workspace=ws, command_runner=cmd)
