...
def _make_handler(
    ...
    *,
    ...
    /api/jobs/{id}/question`` and ``POST /api/jobs/{id}/answer`` are
    forwarded to the matching ``/jobs/{id}/question|answer`` interactive
    deliver routes; with either unset all five stay unavailable and answer
    ``501``. The dispatch token is only ever sent to ``dispatch_url`` —
